...
#' @param IncurredValidation Incurred Validation triangle.
...
MainResults_Incurred=function(CompanyCode,CumulativeValidation,IncurredValidation,Model){
  Data1=CompanyCode
  Data2=sum(IncurredValidation[,dim(IncurredValidation)[1]]) - LastDiagonal(CumulativeValidation)
  Data3=mean(Model$SampledReserves)
  Data4=sum(IncurredValidation[,dim(IncurredValidation)[1]])
